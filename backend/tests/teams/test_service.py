...
Для корректной работы SQLAlchemy mapper нужно импортировать все связанные модели
до инициализации ORM-объектов.
"""
...
async def test_create_team() -> None:
    ...
    # Инициализируем все связанные модели для SQLAlchemy mapper
...
async def test_join_team_valid_code() -> None:
    ...
    from app.teams.service import join_team
...
async def test_remove_member_last_owner_raises() -> None:
    ...
    from app.teams.models import TeamMember, TeamRole
    from app.teams.service import remove_member
...
async def test_invite_code_uses_secrets_token_urlsafe() -> None:
    ...
    with patch("app.teams.service.secrets.token_urlsafe", return_value=generated_token) as mock_token:
...
def test_assignee_membership() -> None:
    ...
    try:
        import app.tasks.service  # noqa: F401
